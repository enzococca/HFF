            ...
            for sing_rec in range(len(data_list_toimp)):
                try:
                    data = self.DB_MANAGER_write.insert_anc_values(
                        ...
                        # biblio, storage_ and box are required by
                        # ANC.__init__ (64 positional args). getattr
                        # fallback keeps imports from pre-biblio/pre-box
                        # source schemas working.
                        getattr(data_list_toimp[sing_rec], 'biblio', None),
                        getattr(data_list_toimp[sing_rec], 'storage_', None),
                        getattr(data_list_toimp[sing_rec], 'box', None))
