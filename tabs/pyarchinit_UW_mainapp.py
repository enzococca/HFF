...
class pyarchinit_UW(QDialog, Ui_DialogUW):
	MSG_BOX_TITLE = "PyArchInit - UW form"
	DATA_LIST = []
	DATA_LIST_REC_CORR = []
	DATA_LIST_REC_TEMP = []
	REC_CORR = 0
	REC_TOT = 0
	STATUS_ITEMS = {"b": "Usa", "f": "Check", "n": "New Record"}
	BROWSE_STATUS = "b"
	SORT_MODE = 'asc'
	SORTED_ITEMS = {"n": "Not in order", "o": "Order"}
	SORT_STATUS = "n"
	...
	def charge_list(self):
		...
		#self.comboBox_sito_rappcheck.addItems(sito1_vl)
		...
		##############################################################################################	
		wind_vl = self.UTILITY.tup_2_list_III(self.DB_MANAGER.group_by('dive_log', 'wind', 'UW'))
		try:
			wind_vl.remove('')
		except Exception, e:
			if str(e) == "list.remove(x): x not in list":
		...
		wind_vl.sort()
		self.comboBox_wind.addItems(wind_vl)
	...
	def customize_GUI(self):
		...
		self.iconListWidget.setIconSize(QtCore.QSize(430, 570))
		...
		self.iconListWidget_2.setIconSize(QtCore.QSize(430, 570))
		self.iconListWidget_2.setMovement(QtGui.QListView.Snap)
		self.iconListWidget_2.setResizeMode(QtGui.QListView.Adjust)
		self.iconListWidget_2.setLayoutMode(QtGui.QListView.Batched)
		...
		self.iconListWidget_2.setSelectionMode(QtGui.QAbstractItemView.SingleSelection)
		self.connect(self.iconListWidget_2, SIGNAL("itemDoubleClicked(QListWidgetItem *)"),self.openWide_image)
	...
	def openWide_image(self):
		...
		for item_2 in items_2:
			...
			dlg_2.show_image(unicode(file_path_2))
			dlg_2.exec_()	
		...
		if self.BROWSE_STATUS == "b":
			if self.data_error_check() == 0:
				if self.records_equal_check() == 1:
					...
					QMessageBox.warning(self, 'Warning',"The record has been changed. Do you want to save changes?",  QMessageBox.Ok)
		else:
			if self.data_error_check() == 0:
	...
	def insert_new_rec(self):
		...
		try:
			...
			unicode(self.comboBox_area_reference.currentText()), 						#1 - Sito
			unicode(self.comboBox_diver.currentText()), 										#3 - US
			unicode(self.comboBox_buddy.currentText()), 					#4 - Definizione stratigrafica
			unicode(self.comboBox_additional_diver.currentText()), 					#5 - Definizione intepretata
			unicode(self.comboBox_standby_diver.currentText()),					#6 - descrizione
			unicode(self.textEdit_task.toPlainText()),
			unicode(self.textEdit_result.toPlainText()),				#
			unicode(self.comboBox_tender.currentText()), 						#11 - fase finale
			unicode(self.lineEdit_bar_start.text()),						#12 - scavato
			unicode(self.lineEdit_bar_end.text()),							#13 - attivita  
			unicode(self.lineEdit_temperature.text()),								#14 - anno scavo
			unicode(self.lineEdit_visibility.text()), 						#15 - metodo
			unicode(self.comboBox_currents.currentText()),													#16 - inclusi
			unicode(self.comboBox_wind.currentText()),													#17 - campioni
			unicode(self.lineEdit_breathing_mix.text()),													#18 - rapporti
			unicode(self.lineEdit_max_depth.text()),						#19 - data schedatura
			unicode(self.lineEdit_surface_interval.text()),					#20 - schedatore
			unicode(self.textEdit_comments.toPlainText()),					#21 - formazione
			unicode(self.lineEdit_bottom_time.text()),				#22 - conservazione
			photo_nbr,					#24 - consistenza
			video_nbr,							#25 - struttura
			unicode(self.lineEdit_camera_of.text()),
			unicode(self.lineEdit_time_in.text()),						#9 - fase iniziale
			unicode(self.lineEdit_time_out.text()),
			unicode(self.lineEdit_date.text()),
			years, #26 - continuita  periodo
			unicode(self.lineEdit_dp.text()),														#27 - order layer
			unicode(photo),
			unicode(video),
			unicode(self.lineEdit_layer.text()),
			unicode(self.comboBox_sito.currentText())
				)
			#todelete
			...
			QMessageBox.warning(self, "Error", "Insert error 2 \n"+str(e),  QMessageBox.Ok)
			return 0
...
##
	...
	def data_error_check(self):
		...
		if EC.data_is_empty(unicode(self.comboBox_area_reference.currentText())) == 0:
...
##					QMessageBox.warning(self, "Errore", str(e),  QMessageBox.Ok)
	...
	def on_pushButton_new_search_pressed(self):
		if self.BROWSE_STATUS != "f" and self.check_record_state() == 1:
			...
			if self.BROWSE_STATUS != "f":
				...
				self.setComboBoxEnable(["self.comboBox_diver"],"True")
				self.setComboBoxEditable(["self.comboBox_diver"],1)
				self.setTableEnable(["self.tableWidget_video", "self.tableWidget_photo"], "True")
				###
				self.label_status.setText(self.STATUS_ITEMS[self.BROWSE_STATUS])
				self.set_rec_counter('','')
				self.label_sort.setText(self.SORTED_ITEMS["n"])
	...
	def on_pushButton_search_go_pressed(self):
		if self.BROWSE_STATUS != "f":
			...
			self.TABLE_FIELDS[1]  : "'"+unicode(self.comboBox_area_reference.currentText())+"'",
			self.TABLE_FIELDS[2]  : "'"+unicode(self.comboBox_diver.currentText())+"'",	#2 - Area
			self.TABLE_FIELDS[3]  : "'"+unicode(self.comboBox_buddy.currentText())+"'",																					#3 - US
			self.TABLE_FIELDS[4]  : "'"+unicode(self.comboBox_additional_diver.currentText())+"'",							#4 - Definizione stratigrafica		self.TABLE_FIELDS[4]  : "'"+unicode(self.lineEdit__diver_3.text())+"'",						#5 - Definizione intepretata
			self.TABLE_FIELDS[5]  : "'"+unicode(self.comboBox_standby_diver.currentText())+"'",									#6 - descrizione
			self.TABLE_FIELDS[6]  : unicode(self.textEdit_task.toPlainText()),								#7 - interpretazione
			self.TABLE_FIELDS[7]  : unicode(self.textEdit_result.toPlainText()),								#8 - periodo inizial						#11 - fase finale
			self.TABLE_FIELDS[8] : "'"+unicode(self.comboBox_tender.currentText())+"'",								#12 - scavato 
			self.TABLE_FIELDS[9] : "'"+unicode(self.lineEdit_bar_start.text())+"'",									#13 - attivita  
			self.TABLE_FIELDS[10] : "'"+unicode(self.lineEdit_bar_end.text())+"'",										#14 - anno scavo
			self.TABLE_FIELDS[11] : "'"+unicode(self.lineEdit_temperature.text())+"'", 								#15 - metodo
			self.TABLE_FIELDS[12] : "'"+unicode(self.lineEdit_visibility.text())+"'",								#16 - data schedatura
			self.TABLE_FIELDS[13] : "'"+unicode(self.comboBox_currents.currentText())+"'",							#17 - schedatore
			self.TABLE_FIELDS[14] : "'"+unicode(self.comboBox_wind.currentText())+"'",							#18 - formazione
			self.TABLE_FIELDS[15] : "'"+unicode(self.lineEdit_breathing_mix.text())+"'",						#19 - conservazione
			self.TABLE_FIELDS[16] : "'"+unicode(self.lineEdit_max_depth.text())+"'",								#20 - colore
			self.TABLE_FIELDS[17] : "'"+unicode(self.lineEdit_surface_interval.text())+"'",							#21 - consistenza
			self.TABLE_FIELDS[18] : unicode(self.textEdit_comments.toPlainText()),
			self.TABLE_FIELDS[19] : "'"+unicode(self.lineEdit_bottom_time.text())+"'",#22 - struttura
			self.TABLE_FIELDS[20] : photo_nbr,								#23 - codice_periodo
			self.TABLE_FIELDS[21] : video_nbr,									#24 - order layer
			self.TABLE_FIELDS[22] : "'"+unicode(self.lineEdit_camera_of.text())+"'"	,									#24 - order layer
			self.TABLE_FIELDS[23] : "'"+unicode(self.lineEdit_time_in.text())+"'"	,						#24 - order layer
			self.TABLE_FIELDS[24] : "'"+unicode(self.lineEdit_time_out.text())+"'",
			self.TABLE_FIELDS[25] : "'"+unicode(self.lineEdit_date.text())+"'",
			self.TABLE_FIELDS[26] : years,
			self.TABLE_FIELDS[29] : "'"+unicode(self.lineEdit_dp.text())+"'",
			self.TABLE_FIELDS[30] : "'"+unicode(self.lineEdit_layer.text())+"'",
			self.TABLE_FIELDS[31] : "'"+unicode(self.comboBox_sito.currentText())+"'"
			}	
...
##			eval(cmd)
	...
	def fill_fields(self, n=0):
		...
		try:
			...
			unicode(self.comboBox_area_reference.setEditText(self.DATA_LIST[self.rec_num].area_id))
			unicode(self.comboBox_diver.setEditText(self.DATA_LIST[self.rec_num].diver_1))#2 - Area
			unicode(self.comboBox_buddy.setEditText(self.DATA_LIST[self.rec_num].diver_2))
			unicode(self.comboBox_additional_diver.setEditText(self.DATA_LIST[self.rec_num].diver_3))
			unicode(self.comboBox_standby_diver.setEditText(self.DATA_LIST[self.rec_num].standby_diver))
			unicode(self.textEdit_task.setText(self.DATA_LIST[self.rec_num].task))
			unicode(self.textEdit_result.setText(self.DATA_LIST[self.rec_num].result))
			unicode(self.comboBox_tender.setEditText(self.DATA_LIST[self.rec_num].tender))
			unicode(self.lineEdit_bar_start.setText(self.DATA_LIST[self.rec_num].bar_start))
			unicode(self.lineEdit_bar_end.setText(self.DATA_LIST[self.rec_num].bar_end))
			unicode(self.lineEdit_temperature.setText(self.DATA_LIST[self.rec_num].temperature))
			unicode(self.lineEdit_visibility.setText(self.DATA_LIST[self.rec_num].visibility))
			unicode(self.comboBox_currents.setEditText(self.DATA_LIST[self.rec_num].current_))
			unicode(self.comboBox_wind.setEditText(self.DATA_LIST[self.rec_num].wind))
			unicode(self.lineEdit_breathing_mix.setText(self.DATA_LIST[self.rec_num].breathing_mix))
			unicode(self.lineEdit_max_depth.setText(self.DATA_LIST[self.rec_num].max_depth))
			unicode(self.lineEdit_surface_interval.setText(self.DATA_LIST[self.rec_num].surface_interval))
			unicode(self.textEdit_comments.setText(self.DATA_LIST[self.rec_num].comments_))
			unicode(self.lineEdit_bottom_time.setText(self.DATA_LIST[self.rec_num].bottom_time))
			...
			unicode(self.lineEdit_camera_of.setText(self.DATA_LIST[self.rec_num].camera_of))
			unicode(self.lineEdit_time_in.setText(self.DATA_LIST[self.rec_num].time_in))
			unicode(self.lineEdit_time_out.setText(self.DATA_LIST[self.rec_num].time_out))
			...
			unicode(self.lineEdit_dp.setText(self.DATA_LIST[self.rec_num].dp))
			...
			unicode(self.lineEdit_layer.setText(self.DATA_LIST[self.rec_num].layer))
			unicode(self.comboBox_sito.setEditText(self.DATA_LIST[self.rec_num].sito))
	...
	def generate_list_pdf(self):
		...
		for i in range(len(self.DATA_LIST)):
			...
			unicode(self.DATA_LIST[i].area_id),									#2 - Area 						#1 - Sito
			unicode(self.DATA_LIST[i].diver_1), 										#3 - US
			unicode(self.DATA_LIST[i].diver_2), 					#4 - Definizione stratigrafica
			unicode(self.DATA_LIST[i].diver_3), 					#5 - Definizione intepretata
			unicode(self.DATA_LIST[i].standby_diver),					#6 - descrizione
			unicode(self.DATA_LIST[i].task),
			unicode(self.DATA_LIST[i].result),		#7 - interpretazione
			unicode(self.DATA_LIST[i].tender), 						#11 - fase finale
			unicode(self.DATA_LIST[i].bar_start),						#12 - scavato
			unicode(self.DATA_LIST[i].bar_end),							#13 - attivita  
			unicode(self.DATA_LIST[i].temperature),								#14 - anno scavo
			unicode(self.DATA_LIST[i].visibility), 						#15 - metodo
			unicode(self.DATA_LIST[i].current_),													#16 - inclusi
			unicode(self.DATA_LIST[i].wind),													#17 - campioni
			unicode(self.DATA_LIST[i].breathing_mix),													#18 - rapporti
			unicode(self.DATA_LIST[i].max_depth),						#19 - data schedatura
			unicode(self.DATA_LIST[i].surface_interval),					#20 - schedatore
			unicode(self.DATA_LIST[i].comments_),					#21 - formazione
			unicode(self.DATA_LIST[i].bottom_time),				#22 - conservazione
			int(self.DATA_LIST[i].photo_nbr),					#24 - consistenza
			int(self.DATA_LIST[i].video_nbr),								#25 - struttura
			unicode(self.DATA_LIST[i].camera_of),
			unicode(self.DATA_LIST[i].time_in),						#9 - fase iniziale
			unicode(self.DATA_LIST[i].time_out), 
			unicode(self.DATA_LIST[i].date_),
			unicode(self.DATA_LIST[i].years),
			unicode(self.DATA_LIST[i].dp),
			unicode(self.DATA_LIST[i].photo_id),
			unicode(self.DATA_LIST[i].video_id),
			unicode(self.DATA_LIST[i].sito),
			unicode(self.DATA_LIST[i].layer)	#29 - documentazione
		])
		return data_list
	...
	def set_LIST_REC_TEMP(self):
		...
		unicode(self.comboBox_standby_diver.currentText()),					#6 - descrizione
		unicode(self.textEdit_task.toPlainText()),
        unicode(self.textEdit_result.toPlainText()),		#7 - interpretazione
		unicode(self.comboBox_tender.currentText()), 						#11 - fase finale
		unicode(self.lineEdit_bar_start.text()),						#12 - scavato
		unicode(self.lineEdit_bar_end.text()),							#13 - attivita  
		unicode(self.lineEdit_temperature.text()),								#14 - anno scavo
		...
		unicode(self.textEdit_comments.toPlainText()),					#21 - formazione
		unicode(self.lineEdit_bottom_time.text()),				#22 - conservazione
		unicode(self.lineEdit_photo_nbr.text()),					#24 - consistenza
		unicode(self.lineEdit_video_nbr.text()),								#25 - struttura
		unicode(self.lineEdit_camera_of.text()),
		unicode(self.lineEdit_time_in.text()),						#9 - fase iniziale
		unicode(self.lineEdit_time_out.text()), 
		unicode(self.lineEdit_date.text()),
		unicode(self.comboBox_years.currentText()),
		unicode(self.lineEdit_dp.text()),
		unicode(photo_id),
		unicode(video_id),
		unicode(self.lineEdit_layer.text()),
		unicode(self.comboBox_sito.currentText()),	
