...
from ..modules.utility.VideoPlayerPottery_conn import VideoPlayerWindow
...
class hff_system__Pottery_CON(QDialog, MAIN_DIALOG_CLASS):
    L=QgsSettings().value("locale/userLocale")[0:2]
    MSG_BOX_TITLE = "HFF - Pottery Conservation"
    DATA_LIST = []
    DATA_LIST_REC_CORR = []
    DATA_LIST_REC_TEMP = []
    REC_CORR = 0
    ...
    UTILITY = Utility()
    DB_MANAGER = ""
    TABLE_NAME = 'pottery_con'
    MAPPER_TABLE_CLASS = "POT_c"
    NOME_SCHEDA = "Pottery Conservation"
    ID_TABLE = "id_pot"
    ID_SITO ="site"
    CONVERSION_DICT = {
    ID_TABLE:ID_TABLE,
    "Site":"site",
    "PotteryId": "pottery_id",
        ...
        if os.path.exists(path_key):
            ...
            with open(path_key, 'r') as f:
                ...
                try:
                    ...
                    reply = QMessageBox.question(self, 'Warning', 'Apikey noy valid' + '\n'
                                                 + 'Click ok to insert the key again or cancel to exit',
                                                 QMessageBox.Ok | QMessageBox.Cancel)
                    if reply == QMessageBox.Ok:

                        api_key, ok = QInputDialog.getText(self, 'Apikey gpt', 'Insert a valid apikey:')
                        if ok:
            ...
            api_key, ok = QInputDialog.getText(self, 'Apikey gpt', 'Insert apikey:')
            if ok:
                # Salva la nuova API Key nel file
                with open(path_key, 'w') as f:
                    f.write(api_key)
    ...
    def dropEvent(self, event):
        ...
        accepted_formats = ["jpg", "jpeg", "png", "tiff", "tif", "bmp", "mp4", "avi", "mov", "mkv", "flv", "obj", "stl", "ply", "fbx", "3ds"]
    ...
    def load_and_process_image(self, filepath):
        ...
        if thumb_path_str=='':
            if self.L=='it':
                QMessageBox.information(self, tr('title_info'), "devi settare prima la path per salvare le thumbnail e i video. Vai in impostazioni di sistema/ path setting ")
            elif self.L=='de':
                QMessageBox.information(self, tr('title_info'), "müssen Sie zuerst den Pfad zum Speichern der Miniaturansichten und Videos festlegen. Gehen Sie zu System-/Pfad-Einstellung")
            else:
    ...
    def on_pushButton_assigntags_pressed(self):
        ...
        all_us = self.DB_MANAGER.query('POT_c')
        # Crea un QListWidget
        ...
        for us in all_us:
            ...
            item_string = f"{us.site} - {us.pottery_id}"
    ...
    def remove_item_from_table(self, item_text, table):
        for row in range(table.rowCount() - 1, -1, -1):  # Start from the last row
            if table.item(row, 0).text() == item_text:
                table.removeRow(row)
            ...
            if self.L == 'it':
                ...
                if msg == QMessageBox.Cancel:
                    ...
                    for item in items_selected:
                        ...
                        #self.remove_item_from_table(id_orig_item, uw().tablewidget_photo )
            ...
            for i in self.record_us_list:
                ...
                if us_list:
                    ...
                    for us_id in us_list:
                        ...
                        search_dict_us = {'id_pot': us_id}
                        search_dict_us = u.remove_empty_items_fr_dict(search_dict_us)

                        # Query the US table
                        us_data = self.DB_MANAGER.query_bool(search_dict_us, "POT_c")
                    ...
                    if us_names:
                        item.setText(item.text() + " - POT_CON: " + ', '.join(us_names))
                    else:
    ...
    def update_list_widget_item(self,item):
        ...
        if mediatoentity_data:
            ...
            if us_data:
                item.setText(item.text() + " - POT_CON: " + str(us_data[0].pottery_id))
            else:
    ...
    def charge_list(self):
        ...
        # lista sito
        author_vl = self.UTILITY.tup_2_list_III(self.DB_MANAGER.group_by('pottery_con', 'author', 'POT_c'))
        try:
            author_vl.remove('')
        except Exception as e:
            if str(e) == "list.remove(x): x not in list":
    ...
    def insert_new_rec(self):
        ...
        try:
            data = self.DB_MANAGER.insert_potterycon_values(
    ...
    def fill_fields(self, n=0):
        ...
        try:
            ...
            str(self.comboBox_artefact.setEditText(self.DATA_LIST[self.rec_num].pottery_id))  # 1 - Sito
            ...
            self.tableInsertData("self.tableWidget_conserved_elements", self.DATA_LIST[self.rec_num].conserved_element)
    ...
    def generate_list_foto(self):
        ...
        for i in range(len(self.DATA_LIST)):
            ...
            search_dict = {'id_entity': "'"+ str(eval("self.DATA_LIST[i].id_pot"))+"'", 'entity_type' : "'POT_CON'"}
    ...
    def generate_list_pdf(self):
        ...
        for i in range(len(self.DATA_LIST)):
            data_list.append([
                ...
                str(self.DATA_LIST[i].observation),
                str(self.DATA_LIST[i].conserved_element),
    ...
    def loadMediaPreview(self, mode = 0):
        ...
        if mode == 0:
            ...
            search_dict = {'id_entity'  : "'"+str(eval("self.DATA_LIST[int(self.REC_CORR)]." + self.ID_TABLE))+"'", 'entity_type' : "'POT_CON'"}
    ...
    def on_pushButton_excel_pressed(self):
        try:
            ...
            rows=self.DB_MANAGER.select_desalination('pottery_id','pottery_con',sito_location)
            ...
            if test_conn == 1:
                ...
                a['Pottery ID'] = a['pottery_id'].mask(a['pottery_id'].duplicated(), '')
                a = a.drop(columns=['pottery_id'])
        ...
        if result:
            ...
            sorted_data_list = sorted(
                ...
                key=lambda record: selected_us_ids.index(
                    record.pottery_id) if record.pottery_id in selected_us_ids else -1
            )

            # Filter out any records that are not in selected_us_ids
            filtered_data_list = [record for record in sorted_data_list if record.pottery_id in selected_us_ids]
...
class USFilterDialog(QDialog):
    ...
    def populate_list_with_us(self):
        ...
        self.us_records = sorted(self.db_manager.query_all('pottery_con'), key=lambda x: x.pottery_id)
    ...
    def update_list_widget(self, records):
        ...
        for us_record in records:
            ...
            checkbox = QCheckBox(f"{us_record.pottery_id}")

            checkbox.us = us_record.pottery_id
    ...
    def filter_list(self, text):
        ...
        filtered_records = [us_record for us_record in self.us_records if
                            str(us_record.pottery_id).startswith(text)]
