...
from ..modules.utility.hff_i18n import HffI18n
from ..modules.utility.hff_form_base import apply_i18n_to_form


class TutorialBrowserWidget(QWidget):
    """Embeddable widget for browsing HFF tutorials.
    ...
    def __init__(self, parent=None):
        ...
        # Plugin directory (this file is in <plugin>/tabs/)
    ...
    def _load_language_preference(self) -> str:
        return QgsSettings().value("HFF/tutorial_language", "en")

    def _save_language_preference(self, lang: str):
        QgsSettings().setValue("HFF/tutorial_language", lang)
    ...
    def load_tutorials(self):
        ...
        for filename in sorted(os.listdir(lang_dir)):
            if filename.endswith('.md'):
                md_files.append((filename, os.path.join(lang_dir, filename)))
    ...
    def filter_tutorials(self, search_text: str):
        ...
        for i in range(self.tutorial_list.count()):
            ...
            visible = (
                search_lower in tutorial['title'].lower()
                or search_lower in tutorial['content'].lower()
            )
    ...
    def on_tutorial_selected(self, current: QListWidgetItem, previous: QListWidgetItem):
        ...
        if tutorial:
            self.content_browser.setHtml(self.markdown_to_html(tutorial['content']))
...
class TutorialViewerDialog(QDialog):
    """Standalone dialog wrapping :class:`TutorialBrowserWidget`."""
    ...
    def __init__(self, parent=None):
        ...
        layout = QVBoxLayout(self)
        self.browser = TutorialBrowserWidget(self)
        layout.addWidget(self.browser)

        button_layout = QHBoxLayout()
        self.refresh_btn = QPushButton("Refresh")
        self.refresh_btn.clicked.connect(self.browser.load_tutorials)
