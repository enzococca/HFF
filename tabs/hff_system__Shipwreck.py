...
from ..modules.utility.hff_system__exp_shipwrecksheet_pdf import generate_SHIPWRECK_pdf
from ..modules.utility.VideoPlayerShipwreck import VideoPlayerWindow
...
class hff_system__Shipwreck(QDialog, MAIN_DIALOG_CLASS, StatisticsMixin):
    ...
    def generate_US(self):
        ...
        record_us_list = []
        sito = self.comboBox_code.currentText()

        # for sing_tags in tags_list:
        search_dict = {'code_id': "'" + str(sito) + "'"
                       }
        j = self.DB_MANAGER.query_bool(search_dict, 'SHIPWRECK')
        record_us_list.append(j)
        # QMessageBox.information(self, 'search db', str(record_us_list))
        us_list = []
        for r in record_us_list:
            us_list.append([r[0].id_shipwreck, 'SHIPWRECK', 'shipwreck_table'])
    ...
    def on_pushButton_assigntags_pressed(self):
        ...
        all_us = self.DB_MANAGER.query('SHIPWRECK')
        # Crea un QListWidget
            ...
            for i in self.record_us_list:
                ...
                if us_list:
                    ...
                    for us_id in us_list:
                        ...
                        search_dict_us = {'id_shipwreck': us_id}
                        search_dict_us = u.remove_empty_items_fr_dict(search_dict_us)

                        # Query the US table
                        us_data = self.DB_MANAGER.query_bool(search_dict_us, "SHIPWRECK")

                        # Se l'US esiste, aggiungi il suo nome alla lista
                        if us_data:
                            us_names.extend([str(us.code_id) for us in us_data])

                    # Se ci sono dei nomi US, aggiungi questi all'elemento
                    if us_names:
                        item.setText(item.text() + " - SHIPWRECK: " + ', '.join(us_names))
                    else:
    ...
    def update_list_widget_item(self, item):
        ...
        if mediatoentity_data:
            ...
            if us_data:
                item.setText(item.text() + " - SHIPWRECK: " + str(us_data[0].code_id))
            else:
        ...
        if result:
            ...
            sorted_data_list = sorted(
                ...
                key=lambda record: selected_us_ids.index(
                    record.code_id) if record.code_id in selected_us_ids else -1
            )

            # Filter out any records that are not in selected_us_ids
            filtered_data_list = [record for record in sorted_data_list if record.code_id in selected_us_ids]
...
class USFilterDialog(QDialog):
    ...
    def populate_list_with_us(self):
        ...
        self.us_records = sorted(self.db_manager.query_all('shipwreck_table'), key=lambda x: x.code_id)
    ...
    def update_list_widget(self, records):
        ...
        for us_record in records:
            ...
            checkbox = QCheckBox(f"{us_record.code_id}")

            checkbox.us = us_record.code_id
    ...
    def filter_list(self, text):
        ...
        filtered_records = [us_record for us_record in self.us_records if
                            str(us_record.code_id).startswith(text)]
