...
class hff_system__ART(QDialog, MAIN_DIALOG_CLASS, StatisticsMixin):
    ...
    ######da finire
    # def parse_graph_data(self,report_text):
    #     # Define a regular expression pattern to match the data format
    #     pattern = r"Category(\d+): (\d+)"
    #
    #     # Find all matches in the report text
    #     matches = re.findall(pattern, report_text)
    #
    #     # Convert matches to a dictionary with category names as keys and values as integers
    #     graph_data = {f"Category{match[0]}": int(match[1]) for match in matches}
    #
    #     return graph_data
    ...
    def on_pushButton_assigntags_pressed(self):
        ...
        all_us = self.DB_MANAGER.query('ART')
        # Crea un QListWidget
            ...
            for i in self.record_us_list:
                ...
                if us_list:
                    ...
                    for us_id in us_list:
                        ...
                        # Query the US table
                        us_data = self.DB_MANAGER.query_bool(search_dict_us, "ART")
                    ...
                    if us_names:
                        item.setText(item.text() + " - ART: " + ', '.join(us_names))
                    else:
    ...
    def update_list_widget_item(self, item):
        ...
        if mediatoentity_data:
            ...
            if us_data:
                item.setText(item.text() + " - ART: " + str(us_data[0].artefact_id))
            else:
    ...
    def plot_chart(self, d, t, yl):
        ...
        #QMessageBox.warning(self, tr('title_alert'), str(teams) ,  QMessageBox.Ok)
    ...
    def on_pushButton_connect_pressed(self):
        ...
        try:
            ...
            if bool(self.DATA_LIST):
                ...
                QMessageBox.warning(self, tr('welcome_hff_user'), tr('welcome_artefact_form'),
                                    QMessageBox.Ok)
                self.charge_list()
    ...
    def on_toolButtonPreviewMedia_toggled(self):
        if self.toolButtonPreviewMedia.isChecked() == True:
            QMessageBox.warning(self, tr('title_message'), "Media Preview Artefact activated. Find images will be displayed in the Media section.", QMessageBox.Ok)
    ...
    def check_record_state(self):
        ...
        if ec == 1:
            ...
            # self.update_if(
                # QMessageBox.warning(self, tr('error', "Error"), "The record has been changed. You want to save the changes?",
                                    # QMessageBox.Ok | QMessageBox.Cancel))
            #self.charge_records()
    ...
    def data_error_check(self):
        ...
        if EC.data_is_empty(str(self.comboBox_artefact.currentText())) == 0:
            QMessageBox.warning(self, tr('warning', "Warning"), "Artefact field. \n This field cannot be empty",  QMessageBox.Ok)
            test = 1
        return test 
    ...
    def on_pushButton_search_go_pressed(self):
        if self.BROWSE_STATUS != "f":
            ...
            if not bool(search_dict):
                QMessageBox.warning(self, tr('warning', "Warning"), u"Not rule has been setted!!!",  QMessageBox.Ok)
            else:
                res = self.DB_MANAGER.query_bool(search_dict, self.MAPPER_TABLE_CLASS)
                if not bool(res):
                    QMessageBox.warning(self, tr('warning', "Warning"), u"No records has been found",  QMessageBox.Ok)
                    self.set_rec_counter(len(self.DATA_LIST), self.REC_CORR+1)
                    self.DATA_LIST_REC_TEMP = self.DATA_LIST_REC_CORR = self.DATA_LIST[0]
                    self.fill_fields(self.REC_CORR)
                    self.BROWSE_STATUS = "b"
...
class USFilterDialog(QDialog):
    ...
    def populate_list_with_us(self):
        ...
        self.us_records = sorted(self.db_manager.query_all('artefact_log'), key=lambda x: x.artefact_id)
