...
from ..modules.utility.VideoPlayerArtefact_conn import VideoPlayerWindow
...
MAIN_DIALOG_CLASS, _ = loadUiType(
    os.path.join(os.path.dirname(__file__), os.pardir, 'gui', 'ui', 'hff_system__ARTLOGCON_ui.ui'))
...
class hff_system__ART_CON(QDialog, MAIN_DIALOG_CLASS):
    L=QgsSettings().value("locale/userLocale")[0:2]
    MSG_BOX_TITLE = "HFF - ARTEFACT Conservation"
    DATA_LIST = []
    DATA_LIST_REC_CORR = []
    DATA_LIST_REC_TEMP = []
    REC_CORR = 0
    ...
    UTILITY = Utility()
    DB_MANAGER = ""
    TABLE_NAME = 'artefact_con'
    MAPPER_TABLE_CLASS = "ART_c"
    NOME_SCHEDA = "Artefact Conservation"
    ID_TABLE = "id_art"
    ID_SITO ="site"
    CONVERSION_DICT = {
    ID_TABLE:ID_TABLE,
    ...
    "ObjPartial": "obj_partial",
    "Author": "author",
    "StartDate": "star_date",
    "EndDate": "end_date",
    "StateConservation": "state_conservation",
    "Observation": "observation",
    #"ConservedElement": "conserved_element",
    "Damage": "damage",
    "Corrosion": "corrosion",
    "Concretion": "concretion",
    "Bio": "bio",
    "Procedure": "procedure",
    "DesalinationDate": "desalination_date"
    }
    SORT_ITEMS = [
                ID_TABLE,
                "Site",
    ...
    TABLE_FIELDS_UPDATE = [
                    "site", "artefact_id", "obj_partial", "author", "star_date", "end_date", "state_conservation", "observation", "damage", "corrosion", "concretion", "bio", "procedure", "desalination_date"]
    TABLE_FIELDS = [
                    "site", "artefact_id", "obj_partial", "author", "star_date", "end_date", "state_conservation", "observation", "damage", "corrosion", "concretion", "bio", "procedure", "desalination_date"]
    ...
    def on_pushButton_assigntags_pressed(self):
        ...
        all_us = self.DB_MANAGER.query('ART_c')
        # Crea un QListWidget
        ...
        for us in all_us:
            ...
            item_string = f"{us.site} - {us.artefact_id}"
    ...
    def on_pushButton_all_images_pressed(self):
        ...
        self.search_field.setPlaceholderText("CFind...Then push start")
    ...
    def load_images(self, filter_text=None):
        ...
        if len(all_images) > 100:
            ...
            for i in self.record_us_list:
                ...
                us_list = [g.id_entity for g in mediatoentity_data if 'ART_CON' in g.entity_type]
                item = QListWidgetItem(str(i.media_filename))
                item.setData(Qt.UserRole, str(i.media_filename))
                icon = QIcon(thumb_path_str + thumb_path)
                item.setIcon(icon)
                if us_list:
                    ...
                    for us_id in us_list:
                        ...
                        search_dict_us = {'id_art': us_id}
                        search_dict_us = u.remove_empty_items_fr_dict(search_dict_us)

                        # Query the US table
                        us_data = self.DB_MANAGER.query_bool(search_dict_us, "ART_c")

                        # Se l'US esiste, aggiungi il suo nome alla lista
                        if us_data:
                            us_names.extend([str(us.artefact_id) for us in us_data])

                    # Se ci sono dei nomi US, aggiungi questi all'elemento
                    if us_names:
                        item.setText(item.text() + " - ART_CON: " + ', '.join(us_names))
                    else:
    ...
    def update_list_widget_item(self, item):
        ...
        if mediatoentity_data:
            ...
            if us_data:
                item.setText(item.text() + " - ART_CON: " + str(us_data[0].artefact_id))
            else:
                item.setText(item.text() + " - ART_CON: Not found")
    ...
    def on_pushButton_connect_pressed(self):
        ...
        try:
            ...
            if bool(self.DATA_LIST):
                ...
                QMessageBox.warning(self, "WELCOME HFF user",
                                    "Welcome in HFF survey:" + " Site form." + " The DB is empty. Push 'Ok' and Good Work!",
                                    QMessageBox.Ok)
                self.charge_list()
    ...
    def customize_GUI(self):
        ...
        self.tableWidget_dpd.setItemDelegateForColumn(0, DateDelegate(self))
        # aggiugno le opzioni
        self.options_conserved_element = ["Windspread", "Patina", "Pitting", "Galvanic", "Crevice", "Selective leanching"]
        #self.options_conserved_element2 = ["Beginning of element", "Fragmented", "Complete"]

        self.delegate = ComboBoxDelegate()
        self.delegate.def_values(self.options_conserved_element)
        self.delegate.def_editable('False')
        self.tableWidget_conserved_elements.setItemDelegateForColumn(0, self.delegate)


        self.tableWidget_ba.resizeColumnsToContents()
        self.tableWidget_ba.setColumnWidth(0, 200)
        self.tableWidget_ba.setColumnWidth(1, 600)

        self.options_bio = ["Shells and molluscs", "Plants", "Other"]
        self.delegate_bio = ComboBoxDelegate()
        self.delegate_bio.def_values(self.options_bio)
        self.delegate_bio.def_editable('False')
        self.tableWidget_ba.setItemDelegateForColumn(0, self.delegate_bio)
        self.tableWidget_ba.setItemDelegateForColumn(1, WordWrapDelegate())

        self.options_mcpd = ["Fissures", "Cracking", "Partial fracturing", "Breacks", "Loss", "Exfoliaton"
            , "Erosion", "Disintegration", "Limination"]
        self.delegate_mcpd = ComboBoxDelegate()
        self.delegate_mcpd.def_values(self.options_mcpd)
        self.delegate_mcpd.def_editable('False')
        self.tableWidget_mpcd.setItemDelegateForColumn(0, self.delegate_mcpd)

        self.options_cs = ["Calcareus concretion", "Metal contretion", "Marine concretion", "Other"]
        self.delegate_cs = ComboBoxDelegate()
        self.delegate_cs.def_values(self.options_cs)
        self.delegate_cs.def_editable('False')
        self.tableWidget_cs.setItemDelegateForColumn(0, self.delegate_cs)
        self.tableWidget_cs.setItemDelegateForColumn(1, WordWrapDelegate())
    ...
    def on_pushButton_go_to_scheda_pressed(self):
        if self.L == 'it':
            QMessageBox.warning(self, "ATTENZIONE",
                                "Se hai modificato il record e non lo hai salvato perderai il dato. Salvare?",
                                QMessageBox.Ok | QMessageBox.Cancel)
        else:
            QMessageBox.warning(self, tr('warning', "Warning"),
                                "If you changed the record and didn't save it, you'll lose the record. Do you want save it?",
                                QMessageBox.Ok | QMessageBox.Cancel)
        ...
        try:
            ...
            e = str(e)
            if self.L == 'it':
                QMessageBox.warning(self, tr('title_alert'), "Non hai selezionato nessuna riga. Errore python: %s " % (str(e)),
                                    QMessageBox.Ok)
            elif self.L == 'de':
                QMessageBox.warning(self, "ACHTUNG", "Keine Spalte ausgewält. Error python: %s " % (str(e)),
                                    QMessageBox.Ok)
            else:
                QMessageBox.warning(self, tr('title_alert'), "You didn't select any row. Python error: %s " % (str(e)),
                                    QMessageBox.Ok)
    ...
    def charge_list(self):
        ...
        # lista sito
        artefact_vl = self.UTILITY.tup_2_list_III(self.DB_MANAGER.group_by('artefact_table', 'artefact_id', 'ART'))
        try:
            artefact_vl.remove('')
        except Exception as e:
            if str(e) == "list.remove(x): x not in list":
        ...
        # lista sito
        author_vl = self.UTILITY.tup_2_list_III(self.DB_MANAGER.group_by('artefact_con', 'author', 'ART_c'))
        try:
            author_vl.remove('')
        except Exception as e:
            if str(e) == "list.remove(x): x not in list":
    ...
    def on_toolButtonPreviewMedia_toggled(self):
        if bool(self.toolButtonPreviewMedia.isChecked()):
            QMessageBox.warning(self, tr('system_message', "Message"),
                                "Pottery Media Preview mode enabled. Pottery images will be displayed in the Media section",
                                QMessageBox.Ok)
    ...
    def remove_row(self, table_name):
        ...
        try:
            ...
            QMessageBox.warning(self, tr('system_message', "Message"), "select a row", QMessageBox.Ok)
    ...
    def on_toolButtonGis_toggled(self):
        if self.toolButtonGis.isChecked() == True:
            QMessageBox.warning(self, tr('system_message', "Message"), "GIS mode activated. From now on what you search will be shown in GIS",
                                QMessageBox.Ok)
        else:
            QMessageBox.warning(self, tr('system_message', "Message"),
                                "GIS mode deactivated. From now on what you search will not be shown in GIS",
                                QMessageBox.Ok)
    ...
    def insert_new_rec(self):
        ##rif_biblio
        dpd = self.table2dict("self.tableWidget_dpd")
        ce = self.table2dict("self.tableWidget_conserved_elements")
        ba = self.table2dict("self.tableWidget_ba")
        mpcd = self.table2dict("self.tableWidget_mpcd")
        cs = self.table2dict("self.tableWidget_cs")
        try:
            data = self.DB_MANAGER.insert_artcon_values(
                self.DB_MANAGER.max_num_id(self.MAPPER_TABLE_CLASS, self.ID_TABLE) + 1,
                str(self.comboBox_site_name.currentText()),
                str(self.comboBox_artefact.currentText()),
                str(self.comboBox_partial_obj.currentText()),
                str(self.comboBox_author.currentText()),
                str(self.dateEdit_start.text()),  # get the text from QLineEdit
                str(self.dateEdit_end.text()),
                str(self.comboBox_st_conservation.currentText()),
                str(self.textEdit_observation.toPlainText()),
                str(mpcd),
                str(ce),
                str(cs),
                str(ba),
                str(self.textEdit_procedure.toPlainText()),
                str(dpd)
    ...
    def on_pushButton_excel_pressed(self):
        try:

            sito_location = str(self.comboBox_site_name.currentText())
            conn = Connection()
            conn_str = conn.conn_str()
            # sito_set= conn.sito_set()
            # sito_set_str = sito_set['sito_set']
            ...
            rows=self.DB_MANAGER.select_desalination('artefact_id','artefact_con',sito_location)


            if test_conn == 1:


                a = pd.DataFrame(rows[0], columns=rows[1])  # changed to use col_names
                # espande il dataframe per avere ogni data in una riga separata
                # Convert strings to lists
                a['Artefact ID'] = a['artefact_id'].mask(a['artefact_id'].duplicated(), '')
                a = a.drop(columns=['artefact_id'])

                a['desalination_date'] = a['desalination_date'].apply(ast.literal_eval)
                ...
                # Creiamo il percorso del file completo aggiungendo il nome del file alla cartella selezionata
                file_path = f"{folder_selected}"
            ...
            QMessageBox.warning(self, tr('error', "Error"), "Error 3 \n" + str(e), QMessageBox.Ok)
    # rif biblio
    def on_pushButton_add_dpd_pressed(self):
        self.insert_new_row('self.tableWidget_dpd')

    def on_pushButton_remove_dpd_pressed(self):
        self.remove_row('self.tableWidget_dpd')

    def on_pushButton_add_ba_pressed(self):
        self.insert_new_row('self.tableWidget_ba')

    def on_pushButton_remove_ba_pressed(self):
        self.remove_row('self.tableWidget_ba')

    def on_pushButton_add_mpcd_pressed(self):
        self.insert_new_row('self.tableWidget_mpcd')

    def on_pushButton_remove_mpcd_pressed(self):
        self.remove_row('self.tableWidget_mpcd')

    def on_pushButton_add_cs_pressed(self):
        self.insert_new_row('self.tableWidget_cs')

    def on_pushButton_remove_cs_pressed(self):
        self.remove_row('self.tableWidget_cs')

    def on_pushButton_add_ce_pressed(self):
        self.insert_new_row('self.tableWidget_conserved_elements')

    def on_pushButton_remove_ce_pressed(self):
        self.remove_row('self.tableWidget_conserved_elements')
    ...
    def on_pushButton_first_rec_pressed(self):
        if self.check_record_state() == 1:
            ...
            try:
                ...
                pass  # QMessageBox.warning(self, tr('error', "Error"), str(e), QMessageBox.Ok)
    ...
    def on_pushButton_delete_pressed(self):
        msg = QMessageBox.warning(self, "Warning!!!",
                                  ...
                                  QMessageBox.Ok | QMessageBox.Cancel)
        if msg == QMessageBox.Cancel:
            QMessageBox.warning(self, tr('title_message'), tr('msg_action_cancelled'))
        else:
            try:
    ...
    def on_pushButton_search_go_pressed(self):
        if self.BROWSE_STATUS != "f":
            QMessageBox.warning(self, tr('warning', "Warning"), "To carry out a new search click on the 'new search' button",
                                QMessageBox.Ok)
        else:
            search_dict = {
                self.TABLE_FIELDS[0]: "'" + str(self.comboBox_site_name.currentText()) + "'",
                self.TABLE_FIELDS[1]: "'" + str(self.comboBox_artefact.currentText()) + "'",  # 2 - Area
                self.TABLE_FIELDS[2]: "'" + str(self.comboBox_partial_obj.currentText()) + "'",  # 3 - US
                self.TABLE_FIELDS[3]: "'" + str(self.comboBox_author.currentText()) + "'",
                self.TABLE_FIELDS[6]: "'" + str(self.comboBox_st_conservation.currentText()) + "'"
            }
            u = Utility()
            search_dict = u.remove_empty_items_fr_dict(search_dict)
    ...
    def update_record(self):
        try:
            ...
            QMessageBox.warning(self, tr('system_message', "Message"),
                                "Encoding problem: accents or characters that are not accepted by the database have been inserted. If you close the window without correcting the errors the data will be lost. Create a copy of everything on a seperate word document. Error :" + str(
                                    e), QMessageBox.Ok)
            return 0
    ...
    def empty_fields(self):
        ##rif_biblio
        dpd = self.tableWidget_dpd.rowCount()
        ce = self.tableWidget_conserved_elements.rowCount()
        ba = self.tableWidget_ba.rowCount()
        mpcd = self.tableWidget_mpcd.rowCount()
        cs = self.tableWidget_cs.rowCount()
        self.comboBox_site_name.setEditText("")

        self.comboBox_artefact.setEditText("")
        self.comboBox_partial_obj.setEditText("")
        self.comboBox_author.setEditText("")
        # self.dateEdit_start.clear()
        # self.dateEdit_end.clear()
        self.comboBox_st_conservation.setEditText("")
        self.textEdit_observation.clear()

        for i in range(mpcd):
            self.tableWidget_mpcd.removeRow(0)
        self.insert_new_row("self.tableWidget_mpcd")  # 16 - inclusi
        for i in range(ce):
            self.tableWidget_conserved_elements.removeRow(0)
        self.insert_new_row("self.tableWidget_conserved_elements")  # 16 - inclusi
        for i in range(cs):
            self.tableWidget_cs.removeRow(0)
        self.insert_new_row("self.tableWidget_cs")  # 16 - inclusi
        for i in range(ba):
            self.tableWidget_ba.removeRow(0)
        self.insert_new_row("self.tableWidget_ba")  # 16 - inclusi
        self.textEdit_procedure.clear()
        for i in range(dpd):
            self.tableWidget_dpd.removeRow(0)
        self.insert_new_row("self.tableWidget_dpd")  # 16 - inclusi

    def fill_fields(self, n=0):
        self.rec_num = n
        # QMessageBox.warning(self, "check fill fields", str(self.rec_num),  QMessageBox.Ok)

        # Controlla se self.rec_num è un indice valido per self.DATA_LIST
        #if self.rec_num >= 0 and self.rec_num < len(self.DATA_LIST):
        try:
            str(self.comboBox_site_name.setEditText(self.DATA_LIST[self.rec_num].site))  # 1 - Sito
            str(self.comboBox_artefact.setEditText(self.DATA_LIST[self.rec_num].artefact_id))  # 1 - Sito
            str(self.comboBox_partial_obj.setEditText(self.DATA_LIST[self.rec_num].obj_partial))  # 1 - Sito
            str(self.comboBox_author.setEditText(self.DATA_LIST[self.rec_num].author))  # 1 - Sito

            str(self.dateEdit_start.setText(self.DATA_LIST[self.rec_num].star_date))  # 1 - Sito
            str(self.dateEdit_end.setText(self.DATA_LIST[self.rec_num].end_date))  # 1 - Sito
            str(self.comboBox_st_conservation.setEditText(
                self.DATA_LIST[self.rec_num].state_conservation))  # 1 - Sito
            str(self.textEdit_observation.setText(self.DATA_LIST[self.rec_num].observation))  # 1 - Sito

            self.tableInsertData("self.tableWidget_mpcd", self.DATA_LIST[self.rec_num].damage)  # 18 - rapporti
            self.tableInsertData("self.tableWidget_conserved_elements", self.DATA_LIST[self.rec_num].corrosion)
            self.tableInsertData("self.tableWidget_cs", self.DATA_LIST[self.rec_num].concretion)  # organici
            self.tableInsertData("self.tableWidget_ba", self.DATA_LIST[self.rec_num].bio)  # inorganici
            str(self.textEdit_procedure.setText(self.DATA_LIST[self.rec_num].procedure))  # 1 - Sito

            self.tableInsertData("self.tableWidget_dpd",
                                 self.DATA_LIST[self.rec_num].desalination_date)  # 16 - inclusi

            # if self.toolButtonPreviewMedia.isChecked() == True:
            self.loadMediaPreview()
            # self.loadMedialist()
        except :
            pass#print('error')  # pass#QMessageBox.warning(self, "Errore Fill Fields", str(e), QMessageBox.Ok)
    ...
    def generate_list_foto(self):
        ...
        for i in range(len(self.DATA_LIST)):
            ...
            search_dict = {'id_entity': "'" + str(eval("self.DATA_LIST[i].id_art")) + "'", 'entity_type': "'ART_CON'"}
    ...
    def generate_list_pdf(self):
        ...
        for i in range(len(self.DATA_LIST)):
            data_list.append([
                ...
                str(self.DATA_LIST[i].artefact_id),
                str(self.DATA_LIST[i].obj_partial),
                str(self.DATA_LIST[i].author),
                str(self.DATA_LIST[i].star_date),
                str(self.DATA_LIST[i].end_date),
                str(self.DATA_LIST[i].state_conservation),
                str(self.DATA_LIST[i].observation),

                str(self.DATA_LIST[i].damage),
                str(self.DATA_LIST[i].corrosion),
                str(self.DATA_LIST[i].concretion),
                str(self.DATA_LIST[i].bio),
                str(self.DATA_LIST[i].procedure),
                str(self.DATA_LIST[i].desalination_date),
            ])
        return data_list
        ...
        if result:
            ...
            sorted_data_list = sorted(
                ...
                key=lambda record: selected_us_ids.index(
                    record.artefact_id) if record.artefact_id in selected_us_ids else -1
            )

            # Filter out any records that are not in selected_us_ids
            filtered_data_list = [record for record in sorted_data_list if record.artefact_id in selected_us_ids]
            ...
            if filtered_data_list:
                ...
                QMessageBox.information(self, tr('title_no_results'), "No records match the selected filters.",
                                        QMessageBox.Ok)
...
class USFilterDialog(QDialog):
    ...
    def populate_list_with_us(self):
        ...
        self.us_records = sorted(self.db_manager.query_all('artefact_con'), key=lambda x: x.artefact_id)
    ...
    def update_list_widget(self, records):
        ...
        for us_record in records:
            ...
            checkbox = QCheckBox(f"{us_record.artefact_id}")

            checkbox.us = us_record.artefact_id
            self.list_widget.addItem(list_item)
            self.list_widget.setItemWidget(list_item, checkbox)

    def filter_list(self, text):
        # Filter US records based on the search text
        filtered_records = [us_record for us_record in self.us_records if
                            str(us_record.artefact_id).startswith(text)]
...
class DateLineEdit(QLineEdit):
    ...
    def update_date(self):
        ...
        self.setText(selected_date.toString("dd-MM-yyyy"))
