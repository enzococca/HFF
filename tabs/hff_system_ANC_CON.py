...
MAIN_DIALOG_CLASS, _ = loadUiType(
    os.path.join(os.path.dirname(__file__), os.pardir, 'gui', 'ui', 'hff_system__ANCHORCON_ui.ui'))
...
class hff_system_ANC_CON(QDialog, MAIN_DIALOG_CLASS):
    L=QgsSettings().value("locale/userLocale")[0:2]
    MSG_BOX_TITLE = "HFF - ANCHOR conservation form"
    DATA_LIST = []
    DATA_LIST_REC_CORR = []
    DATA_LIST_REC_TEMP = []
    REC_CORR = 0
    ...
    UTILITY = Utility()
    DB_MANAGER = ""
    TABLE_NAME = 'anchor_con'
    MAPPER_TABLE_CLASS = "ANC_c"
    NOME_SCHEDA = "Anchor Conservation"
    ID_TABLE = "id_anc"
    ID_SITO ="site"
    ...
    def generate_US(self):
        ...
        search_dict = {'site': "'" + str(sito) + "'",
                       'anchor_id': "'" + str(artefact) + "'"
                       }
        j = self.DB_MANAGER.query_bool(search_dict, 'ANC_c')
        record_us_list.append(j)
        # QMessageBox.information(self, 'search db', str(record_us_list))
        us_list = []
        for r in record_us_list:
            us_list.append([r[0].id_anc, 'ANC_CON', 'anchor_con'])
    ...
    def on_pushButton_assigntags_pressed(self):
        ...
        all_us = self.DB_MANAGER.query('ANC_c')
        # Crea un QListWidget
        ...
        for us in all_us:
            ...
            item_string = f"{us.site} - {us.anchor_id}"
            ...
            for i in self.record_us_list:
                ...
                if us_list:
                    ...
                    for us_id in us_list:
                        ...
                        # Query the US table
                        us_data = self.DB_MANAGER.query_bool(search_dict_us, "ANC_c")

                        # Se l'US esiste, aggiungi il suo nome alla lista
                        if us_data:
                            us_names.extend([str(us.anchor_id) for us in us_data])

                    # Se ci sono dei nomi US, aggiungi questi all'elemento
                    if us_names:
                        item.setText(item.text() + " - ANC_CON: " + ', '.join(us_names))
                    else:
    ...
    def update_list_widget_item(self, item):
        ...
        if mediatoentity_data:
            ...
            if us_data:
                item.setText(item.text() + " - ANC_CON: " + str(us_data[0].anchor_id))
            else:
                item.setText(item.text() + " - ANC_CON: Not found")
    ...
    def customize_GUI(self):
        ...
        # aggiugno le opzioni
        # self.options_conserved_element = ["Rim", "Base", "Neck", "Body", "Handle 1", "Handle 2"]
        ...
        #
        # self.delegate1 = ComboBoxDelegate()
        # self.delegate1.def_values(self.options_conserved_element2)
        # self.delegate1.def_editable('False')
        # self.tableWidget_conserved_elements.setItemDelegateForColumn(1, self.delegate1)
    ...
    def charge_list(self):
        ...
        _cb(self.comboBox_author,    _q('anchor_con', 'author', 'ANC_c'))
    def on_toolButtonPreviewMedia_toggled(self):
        if bool(self.toolButtonPreviewMedia.isChecked()):
            QMessageBox.warning(self, tr('system_message', "Message"),
                                "Anchor Media Preview mode enabled. Anchor images will be displayed in the Media section",
                                QMessageBox.Ok)
    ...
    def insert_new_rec(self):
        ...
        try:
            data = self.DB_MANAGER.insert_anccon_values(
    ...
    def fill_fields(self, n=0):
        ...
        try:
            ...
            str(self.comboBox_artefact.setEditText(self.DATA_LIST[self.rec_num].anchor_id))  # 1 - Sito
            ...
            str(self.comboBox_st_conservation.setEditText(self.DATA_LIST[self.rec_num].state_conservation))  # 1 - Sito
            str(self.textEdit_observation.setText(self.DATA_LIST[self.rec_num].observation))  # 1 - Sito

            self.tableInsertData("self.tableWidget_conserved_elements", self.DATA_LIST[self.rec_num].measure)
            self.tableInsertData("self.tableWidget_mpcd", self.DATA_LIST[self.rec_num].damage)  # 18 - rapporti
            self.tableInsertData("self.tableWidget_cs",
                                 self.DATA_LIST[self.rec_num].concretion)  # organici
            self.tableInsertData("self.tableWidget_ba",
                                 self.DATA_LIST[self.rec_num].bio)  # inorganici
            str(self.textEdit_procedure.setText(self.DATA_LIST[self.rec_num].procedure))  # 1 - Sito

            self.tableInsertData("self.tableWidget_dpd", self.DATA_LIST[self.rec_num].desalination_date)  # 16 - inclusi
    ...
    def generate_list_foto(self):
        ...
        for i in range(len(self.DATA_LIST)):
            ...
            search_dict = {'id_entity': "'" + str(eval("self.DATA_LIST[i].id_anc")) + "'", 'entity_type': "'ANC_CON'"}
    ...
    def on_pushButton_print_pressed(self):
        ...
        if self.checkBox_s_pottery.isChecked():
            try:
                ...
                log_pdf_export_error("Anchor Conservation")
    ...
    def on_pushButton_excel_pressed(self):
        try:
            ...
            rows=self.DB_MANAGER.select_desalination('anchor_id','anchor_con',sito_location)
            ...
            if test_conn == 1:
                ...
                a['Anchor ID'] = a['anchor_id'].mask(a['anchor_id'].duplicated(), '')
                a = a.drop(columns=['anchor_id'])
        ...
        if result:
            ...
            sorted_data_list = sorted(
                ...
                key=lambda record: selected_us_ids.index(
                    record.anchor_id) if record.anchor_id in selected_us_ids else -1
            )

            # Filter out any records that are not in selected_us_ids
            filtered_data_list = [record for record in sorted_data_list if record.anchor_id in selected_us_ids]
...
class USFilterDialog(QDialog):
    ...
    def populate_list_with_us(self):
        ...
        self.us_records = sorted(self.db_manager.query_all('anchor_con'), key=lambda x: x.anchor_id)
    ...
    def update_list_widget(self, records):
        ...
        for us_record in records:
            ...
            checkbox = QCheckBox(f"{us_record.anchor_id}")

            checkbox.us = us_record.anchor_id
    ...
    def filter_list(self, text):
        ...
        filtered_records = [us_record for us_record in self.us_records if
                            str(us_record.anchor_id).startswith(text)]
