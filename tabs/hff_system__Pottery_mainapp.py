...
class hff_system__Pottery(QDialog, MAIN_DIALOG_CLASS, StatisticsMixin):
    ...
    def __init__(self, iface):
        ...
        self.init_statistics(POTTERY_STATS_FIELDS)
    ...
    def generate_US(self):
        ...
        for r in record_us_list:
            us_list.append([r[0].id_rep, 'POTTERY', 'pottery_table'])
    ...
    def on_pushButton_assigntags_pressed(self):
        ...
        all_us = self.DB_MANAGER.query('POTTERY')
        # Crea un QListWidget
    ...
    def on_done_selecting(self):
        ...
        def r_list():
            ...
            us_list = []
            for r in record_us_list:
                us_list.append([r[0].id_rep, 'POTTERY', 'pottrery_table'])
            # QMessageBox.information(self, "Scheda US", str(us_list), QMessageBox.Ok)
            return us_list
    ...
    def load_images(self, filter_text=None):
        ...
        if len(all_images) > 100:
            ...
            for i in self.record_us_list:
                ...
                us_list = [g.id_entity for g in mediatoentity_data if 'POTTERY' in g.entity_type]
                item = QListWidgetItem(str(i.media_filename))
                item.setData(Qt.UserRole, str(i.media_filename))
                icon = QIcon(thumb_path_str + thumb_path)
                item.setIcon(icon)
    ...
    def update_list_widget_item(self, item):
        ...
        if mediatoentity_data:
            ...
            if us_data:
                item.setText(item.text() + " - POTTERY: " + str(us_data[0].artefact_id))
            else:
...
class USFilterDialog(QDialog):
    ...
    def initUI(self):
        ...
        if self.L == 'it':
            ...
            self.setWindowTitle("Filter Pottery Records")  # Set the window title
    ...
    def populate_list_with_us(self):
        ...
        self.us_records = sorted(self.db_manager.query_all('pottery_table'), key=lambda x: x.artefact_id)
