...
CREATE TABLE IF NOT EXISTS public.shipwreck_table (
	...
	consulties character varying(255),
    l numeric(9,4),
    w numeric(9,4),
    d numeric(9,4),
    t numeric(9,4),
...
SELECT id_shipwreck AS id_shipwreck,
	...
	a.depth_quality as depth_quality, a.latitude as latitude, a.longitude as longitude,
	a.position_quality_1 as position_quality_1, a.consulties as consulties
...
CREATE or replace VIEW public.pyarchinit_grabspot_view AS
    ...
    site_table.photolog ,
    site_table.est ,
    site_table.material_c,
    site_table.morphology_c,
    site_table.collection_c ,
    site_table.photo_material,
    site_table.damage,
    site_table.country ,
    grab_spot.gid,
	grab_spot.name_grab,
