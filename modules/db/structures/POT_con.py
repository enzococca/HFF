...
class POT_con(object):

	internal_connection = Connection()
	engine = create_engine(internal_connection.conn_str(), echo=False)

	metadata = MetaData()
	pottery_con = Table('pottery_con', metadata,
						Column('id_pot', Integer, primary_key=True),
						Column('site', Text),
						Column('pottery_id', Text),
						...
						UniqueConstraint('pottery_id', name='pottery_id_unico')
						)
